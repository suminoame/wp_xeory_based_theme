...
if( !function_exists('bzb_breadcrumb') ){
  ...
  {
    $list_item = ( $href === home_url() ) ? '' : '<li itemscope itemtype="https://schema.org/ListItem" itemprop="itemListElement">></li>';
    ...
    $list_item .= ( $href === '' ) ? '' : '</a>';
    $list_item .= '<meta itemprop="position" content="' . $position . '"></li>';
  ...
  function bzb_breadcrumb(){
    ...
    if( is_home() ){
      ...
      // edited same@1.0.0
      if( is_day() ) {
        $item_name .= get_the_time('Y') . '-';
        $item_name .= get_the_time('m') . '-';
        $item_name .= get_the_time('d') . '';
      } elseif( is_month() ) {
        $item_name .= get_the_time('Y') . '-';
        $item_name .= get_the_time('m') . '';
      } elseif( is_year() ) {
